...
const ProductForm = (props) => {
  ...
  const productReducer = (state, action) => {
    ...
    if (action.type === "QUANTITY_INPUT") {
      if (state.rate) {
        return {
          ...state,
          total: state.rate * action.val,
          rate: action.val,
          rateIsValid: !!action.val.trim(),
        };
      }
      return {
        ...state,
        quantity: action.val,
        quantityIsValid: !!action.val.trim(),
      };
    }
    if (action.type === "QUANTITY_BLUR") {
      return {
        ...
        quantityIsValid: !!state.quantity.trim(),
      };
    }
    if (action.type === "RATE_INPUT") {
      if (state.quantity) {
        return {
          ...state,
          total: state.quantity * action.val,
          rate: action.val,
          rateIsValid: !!action.val.trim(),
        };
      }
  ...
  useEffect(() => {
    setformIsValid(
      productState.quantityIsValid &&
        productState.rateIsValid &&
        productState.nameIsValid
    );
  }, [
    productState.quantityIsValid,
    productState.rateIsValid,
      ...
      {showadd ? (
        <form className="addform" onSubmit={addProduct}>
          <div>
            ...
            <div>
              {add && productState.nameIsValid !== true
                ? "Enter a valid name"
                : ""}
            </div>
          </div>
          <div>
            ...
            <div>
              {add && productState.quantityIsValid !== true
          ...
          <div>
            ...
            <div>
              {add && productState.rateIsValid !== true
                ? "Enter a valid rate"
                : ""}
            </div>
          </div>
